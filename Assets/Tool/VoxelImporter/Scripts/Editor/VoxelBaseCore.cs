...
namespace VoxelImporter
{
    public abstract class VoxelBaseCore
    {
        ...
        protected bool LoadVoxelDataFromVOX(BinaryReader br)
        {
            ...
            br.BaseStream.Seek(8, SeekOrigin.Current);  //version
            #region MAIN
            if (!CheckChunk("MAIN"))
            {
            ...
            UInt32 numVoxels = 0;
            VoxelData.Voxel[] voxels = null;
            for (int pack = 0; pack < packCount; pack++)
            {
                if (voxelBase.voxelFileSubIndex == pack)
                {
                    #region SIZE
                    if (!CheckChunk("SIZE"))
                    {
                    ...
                    br.BaseStream.Seek(12, SeekOrigin.Current);
                    numVoxels = br.ReadUInt32();
                    voxels = new VoxelData.Voxel[numVoxels];
        ...
        protected bool LoadVoxelDataFromQB(BinaryReader br)
        {
            ...
            voxelBase.voxelData.qbChunkIndexTable = chunkIndexTable;
            voxelBase.voxelData.qbChunkDataList = chunkDataList;
