    ...
    public class VoxelChunksObjectCore : VoxelBaseCore
    {
        ...
        protected override void CreateChunkData()
        {
            if (voxelObject.splitMode == VoxelChunksObject.SplitMode.QubicleMatrix)
            {
                ...
                var chunkVoxels = new List<int>[voxelData.qbChunkDataList.Count];
                var chunkPalettes = new HashSet<Color>[voxelData.qbChunkDataList.Count];
                for (int i = 0; i < voxelData.qbChunkDataList.Count; i++)
                {
                ...
                {
                    var chunkIndex = voxelData.qbChunkIndexTable.Get(voxelData.voxels[i].position);
                    //voxel
                ...
                }
                {
                    ...
                    for (int i = 0; i < chunkVoxels.Length; i++)
                    {
                        chunkDataList.Add(new ChunkData()
                        {
                            ...
                            name = string.Format("Chunk({0})", voxelData.qbChunkDataList[i].name),
                            area = voxelData.qbChunkDataList[i].area,
                            voxels = chunkVoxels[i],
                            palettes = chunkPalettes[i].ToArray()
                        });
                    }
                    chunkDataList.Sort((a, b) => string.Compare(voxelData.qbChunkDataList[a.position.x].name, voxelData.qbChunkDataList[b.position.x].name));
                }
            }
            else
            {
